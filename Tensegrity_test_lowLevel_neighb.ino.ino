...
void updateMotor() { // steps motors based on time intervals
  ...
  if ( micros() - step_us_ts[TEST_MOTOR] > step_delay[TEST_MOTOR] ) {
    ...
    if ( abs(local_err) < local_threshold ) {
      ...
      moveStepsDown(actuation_step * local_weight);
    ...
    if (diff_neigh > 0) {
      if (neighbour_condition == -1) {
        ...
        moveStepsDown(actuation_step * neigh_weight); // elongates
      ...
      if (neighbour_condition == -1) {
        moveStepsUp(actuation_step * neigh_weight);
        Serial.println("Increase voltage to converge");
      }
      else if (neighbour_condition == 1) {
        moveStepsDown(actuation_step * neigh_weight);
