...
video_output_path = os.path.join(script_dir, 'PositionTrackingVideo.mp4')
...
# Read first frame to setup VideoWriter after resizing
ret, frame = vs.read()
if not ret:
    print("[ERROR] Could not read initial frame from video stream.")
    exit()

frame = imutils.resize(frame, width=600)
frame_height, frame_width = frame.shape[:2]

# Setup video writer (after knowing correct dimensions)
fourcc = cv2.VideoWriter_fourcc(*'avc1')  # More compatible for .mp4
fps = 20.0
...
start_x = 300 #higher --> right 
start_y = 291 #higher --> low

# Main loop
while True:

    # 1️⃣ Receive data from Arduino
    ...
    # 3️⃣ Draw target and starting point
    cross_size = 20
    cross_color = (0, 255, 0)
    thickness = 2
    ...
    cv2.circle(frame, (target_x, target_y), 50, (0, 255, 0), 2)
    cv2.circle(frame, (start_x, start_y), 10, (0, 255, 255), 2)
    #cv2.putText(frame, "Start", (start_x + 10, start_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
...
print("[INFO] Experiment finished. Data saved to Excel and video recorded.")
