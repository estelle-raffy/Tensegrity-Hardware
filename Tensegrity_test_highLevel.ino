...
int global_demand = 0; // sum of own + neigh voltages 
int global_threshold = 0; // happiness range, where not strained/stressed 
int global_weight = 1; // how much neigh affects bhv, 0 = OFF
// should also be turning/overwriting local and neigh weights ON/OFF
...
//  Serial.print(",");
  ...
  // Move motor: this is non-blocking and will handle it's own timing#

  // call higher-level function that may overwrite weights
  // e.g. updateGlobal()---> takes in sum of voltages and check stress levels vs boundaries
  // wants to be more stressed (alert, attention) or less stressed (no need)
  // could sensitize or habituate... with a count and after x repeat, just stops stressing?  
  updateMotor0();
  updateMotor1();
